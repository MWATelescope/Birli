...
impl RADec {
    ...
    /// Given an [`mwalib::MetafitsContext`], make an [Option<RADec>] from
    /// the `(ra|dec)_phase_center_degrees` if these are available, otherwise None
    ...
    /// Given an [`mwalib::MetafitsContext`], make a [RADec] from
